...
if(isset($_POST['submit'])){
   ...
   $update_review = $conn->prepare("UPDATE `$reviews_tables` SET rating = ?, title = ?, description = ? WHERE id = ?");
...
<section class="account-form">
   ...
   <?php
      $select_review = $conn->prepare("SELECT * FROM `$reviews_tables` WHERE id = ? LIMIT 1");
   ...
   <form action="" method="post">
      ...
      <a href="view_post.php?get_id=<?= $fetch_review['post_id']; ?>&table=<?=$table?>" class="option-btn">go back</a>
   </form>
   <?php
         }
      }else{
